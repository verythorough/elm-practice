...
Elm.StringMod = Elm.StringMod || {};
Elm.StringMod.make = function (_elm) {
   "use strict";
   _elm.StringMod = _elm.StringMod || {};
   if (_elm.StringMod.values) return _elm.StringMod.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var leetPairs = _U.list([{ctor: "_Tuple2",_0: "a",_1: "4"}
                           ,{ctor: "_Tuple2",_0: "e",_1: "3"}
                           ,{ctor: "_Tuple2",_0: "g",_1: "6"}
                           ,{ctor: "_Tuple2",_0: "i",_1: "1"}
                           ,{ctor: "_Tuple2",_0: "o",_1: "0"}
                           ,{ctor: "_Tuple2",_0: "s",_1: "5"}
                           ,{ctor: "_Tuple2",_0: "t",_1: "7"}]);
   var symbolStripPairs = _U.list([{ctor: "_Tuple2"
                                   ,_0: "[`\']"
                                   ,_1: ""}
                                  ,{ctor: "_Tuple2",_0: "[^a-zA-Z\\d]+",_1: " "}]);
   var replaceFromTuple = F2(function (_p0,str) {
      var _p1 = _p0;
   ...
   var encodeStr = F2(function (pairs,str) {
      return A3($List.foldl,replaceFromTuple,str,pairs);
   });
   var capHyphenated = F2(function (capFunc,word) {
      return A2($String.contains,"-",word) ? A2($String.join,
      "-",
      A2($List.map,
      capFunc,
      A2($String.split,"-",word))) : capFunc(word);
   });
   var smallWords = _U.list(["a"
                            ,"an"
                            ,"and"
                            ,"at"
                            ,"but"
   ...
   var capWord = function (word) {
      ...
      $Regex.regex("[A-Z]"),
      word) ? word : A2($Basics._op["++"],
      $String.toUpper(A2($String.left,1,word)),
      A2($String.dropLeft,1,word));
   };
   var titleCapWord = function (word) {
      return A2($List.member,
      word,
      smallWords) ? word : capWord(word);
   };
   var convertString = F2(function (converter,str) {
      var stripSplitList = $String.words(A2(encodeStr,
      symbolStripPairs,
      str));
      var _p3 = converter;
      switch (_p3)
      {case "Lowercase": return $String.toLower(str);
         case "Caps": return $String.toUpper(str);
         case "Reverse": return $String.reverse(str);
         case "ReverseCaps":
         return $String.toUpper($String.reverse(str));
         case "Sentence": return capWord(str);
         case "Initial": return A2($String.join,
           " ",
           A2($List.map,capHyphenated(capWord),A2($String.split," ",str)));
         case "Title": var wordList = A2($String.split," ",str);
           var lastIndex = $List.length(wordList) - 1;
           return A2($String.join,
           " ",
           A2($List.indexedMap,
           F2(function (i,s) {
              return _U.eq(i,0) || _U.eq(i,
              lastIndex) ? capWord(s) : titleCapWord(s);
           }),
           wordList));
         case "Snake": return A2($String.join,"_",stripSplitList);
         case "ScreamingSnake": return $String.toUpper(A2($String.join,
           "_",
           stripSplitList));
         case "Kebob": return A2($String.join,"-",stripSplitList);
         case "Train": return A2($String.join,
           "-",
           A2($List.map,capWord,stripSplitList));
         case "Camel": return $String.concat(A2($List.indexedMap,
           F2(function (i,s) {
              return _U.eq(i,0) ? $String.toLower(s) : capWord(s);
           }),
           $String.words(A2(encodeStr,symbolStripPairs,str))));
         case "UpperCamel": return $String.concat(A2($List.map,
           capWord,
           $String.words(A2(encodeStr,symbolStripPairs,str))));
         case "Leet": return A2(encodeStr,
           leetPairs,
           $String.toLower(str));
         case "Studly": return $String.concat(A2($List.indexedMap,
           F2(function (i,s) {
              return _U.eq(A2($Basics._op["%"],i,2),
              0) ? $String.toUpper(s) : $String.toLower(s);
           }),
           A2($String.split,"",str)));
         default:
         return "This converter is not yet defined in StringMod.convertString.";}
   });
   var update = F2(function (action,model) {
      var _p4 = action;
      if (_p4.ctor === "SetInputStr") {
            return {ctor: "_Tuple2"
                   ,_0: _U.update(model,{inputStr: _p4._0})
                   ,_1: $Effects.none};
         } else {
            return {ctor: "_Tuple2"
                   ,_0: _U.update(model,
                   {outputStr: A2(convertString,_p4._0,model.inputStr)})
                   ,_1: $Effects.none};
         }
   });
   var ConvertBtn = F2(function (a,b) {
      return {converter: a,label: b};
   });
   var listConvertBtns = _U.list([A2(ConvertBtn,
                                 "Lowercase",
                                 "lowercase")
                                 ,A2(ConvertBtn,"Caps","ALL CAPS")
                                 ,A2(ConvertBtn,"Reverse","sdrawkcaB")
                                 ,A2(ConvertBtn,"ReverseCaps","SDRAWKCAB")
                                 ,A2(ConvertBtn,"Sentence","Sentence case")
                                 ,A2(ConvertBtn,"Initial","Initial Case")
                                 ,A2(ConvertBtn,"Title","Title Case (AP)")
                                 ,A2(ConvertBtn,"Snake","snake_case")
                                 ,A2(ConvertBtn,"ScreamingSnake","SCREAMING_SNAKE_CASE")
                                 ,A2(ConvertBtn,"Kebob","kebob-case")
                                 ,A2(ConvertBtn,"Train","Train-Case")
   ...
   var SetInputStr = function (a) {
      return {ctor: "SetInputStr",_0: a};
   };
   var defaultValue = function (str) {
   ...
   var onInput = F2(function (address,wrap) {
      ...
      function (val) {
         return A2($Signal.message,address,wrap(val));
      });
   });
   var viewButton = F2(function (address,btn) {
      var buttonId = A2($Basics._op["++"],
      $String.toLower(btn.converter),
      "-btn");
      return A2($Html.label,
      _U.list([$Html$Attributes.$for(buttonId)]),
      _U.list([A2($Html.input,
              _U.list([$Html$Attributes.id(buttonId)
                      ,$Html$Attributes.type$("radio")
                      ,$Html$Attributes.value(btn.converter)
                      ,A2($Html$Events.onClick,address,Convert(btn.converter))
                      ,$Html$Attributes.name("converters")]),
              _U.list([]))
              ,A2($Html.span,_U.list([]),_U.list([$Html.text(btn.label)]))]));
   });
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.h2,
              _U.list([]),
              _U.list([$Html.text("StringMod")]))
              ,A2($Html.p,
              _U.list([]),
              _U.list([$Html.text("Enter a string and choose a style.")]))
              ,A2($Html.input,
              _U.list([$Html$Attributes.type$("text")
                      ,A2(onInput,address,SetInputStr)
                      ,defaultValue(model.inputStr)
                      ,$Html$Attributes.autofocus(true)]),
              _U.list([]))
              ,A2($Html.div,
              _U.list([]),
              A2($List.map,viewButton(address),listConvertBtns))
              ,A2($Html.h3,_U.list([]),_U.list([$Html.text("Result:")]))
              ,A2($Html.input,
              _U.list([$Html$Attributes.type$("text")
                      ,$Html$Attributes.readonly(true)
                      ,$Html$Attributes.value(model.outputStr)]),
              _U.list([]))]));
   });
   var initialModel = {inputStr: "this is a sample",outputStr: ""};
   var Model = F2(function (a,b) {
      return {inputStr: a,outputStr: b};
   });
   var app = $StartApp.start({view: view
                             ,update: update
                             ,init: {ctor: "_Tuple2",_0: initialModel,_1: $Effects.none}
                             ,inputs: _U.list([])});
   var main = app.html;
   return _elm.StringMod.values = {_op: _op
                                  ,main: main
                                  ,app: app
                                  ,Model: Model
                                  ...
                                  ,update: update
                                  ,listConvertBtns: listConvertBtns
                                  ,convertString: convertString
                                  ,capWord: capWord
                                  ,titleCapWord: titleCapWord
                                  ,smallWords: smallWords
                                  ,capHyphenated: capHyphenated
                                  ,encodeStr: encodeStr
                                  ,replaceFromTuple: replaceFromTuple
                                  ,symbolStripPairs: symbolStripPairs
                                  ,leetPairs: leetPairs};
};
